    ...
    <div class="main-container">
        <div class="header-container">
            <div class="header-title-container">
                ...
                <label class="pull-right">API</label>
            </div>
        </div>
...
<script>
    ...
    $(document).ready(function(){
        ...
        $("#buton_admin_add").click(function(){
            ...
            $.ajax({
                method: "POST",
                url: "<?php echo Uri::create('api/home/add')?>", 
        ...
        $(document).on('click','.edit_data', function() {
            ...
            $.ajax({
                method: "POST",
                dataType: 'JSON',
                url: "<?php echo Uri::create('api/home/detail')?>", 
        ...
        $(document).on('click','#buton_admin_edit', function() {
            ...
            $.ajax({
                method: "POST",
                url: "<?php echo Uri::create('api/home/edit')?>", 
        ...
        $('#btn_delete').click(function(){
            ...
            if(id.length === 0) {
                ...
                if (confirm("Are you sure you want to delete this?")) {
                    $.ajax({
                            method:'POST',
                            url:'<?php echo Uri::create('api/home/multidelete')?>',
                            data:{id:id},
                            success: function() {
    ...
    function showlistuser() { 
        var sEmp      = $("input[name='sEmp']").val();
        var sName     = $("input[name='sName']").val();
        var sEmail    = $("input[name='sEmail']").val();
        var sPhone    = $("input[name='sPhone']").val();
        var sDepar    = $("#sDepars option:selected").val();
        var datefrom  = $("input[name='datefrom']").val();
        var dateto    = $("input[name='dateto']").val();

        $.ajax({
                method: "POST",
                url: "<?php echo Uri::create('api/home/getlist')?>",
                data: {sEmp:sEmp, sName:sName, sEmail:sEmail, sPhone:sPhone, sDepar:sDepar, datefrom:datefrom, dateto:dateto,page:page},
                success: function(data) {
                        var output      = '';
                        var paginate    = '';
                        if (data.errors = []) {
                            output = 'data not fould';
                        }
                        $.each(data.list, function (index, value){ 
                            console.log(data.page_info);
                            output += '<tr id="'+value['id']+'"><td class="col-sm-1"><input type="checkbox" name="user_id[]" id="user_id" class="delete_customer" value="'+value['id']+'"></td><td class="col-sm-1">'+value['create_time']+'</td><td class="col-sm-1">'+value['employee_id']+'</td><td class="col-sm-2">'+value['name']+'</td><td class="col-sm-1">'+value['email']+'</td><td class="col-sm-2">'+value['phone_num']+'</td><td class="col-sm-2">'+value['department_name']+'</td><td class="col-sm-1"><button type="button" class="btn btn-xs btn-info edit_data" name="edit" id="'+value['id']+'" data-toggle="modal"  data-target="#admin-detail-modal">EDIT</button></td><td class="col-sm-1"><button type="button" class="btn btn-xs btn-info" name="reset_password" id="'+value['id']+'" data-toggle="modal" data-target="#admin-detail-reset-password">Reset Password</button></td></tr>';

                        });
                        $('#listuser').html(output); 

                        for(var i = 1; i <= data.page_info.total_page; i++) {

                                if (i == data.page_info.current_page) {
                                    paginate += '<li class="active"><a onclick="setPage('+i+')" href="#">'+i+'</a></li>';
                                } else {
                                    paginate += '<li><a onclick="setPage('+i+')" href="#">'+i+'</a></li>';
                                }
                        }
